...
import { BlogCard, BlogCardDescription, BlogCardHeader, BlogContent, Header, Root } from "./PortfolioProjects.styles";
		...
		>
			{Array.from(projects).map((project, index) => (
				<Grid item xs={12} sm={6} md={4} lg={4} key={index}>
					<BlogCard onClick={() => {handleProjectOnClick(project.redirectionLink ?? "")}} elevation={2} style={{ backgroundImage: `url(${projectImages[index] ?? ""})`}}/>
					<BlogContent>
						<BlogCardHeader>{project?.title}</BlogCardHeader>
						<BlogCardDescription>{project?.description}</BlogCardDescription>
						<a href={project?.redirectionLink}>View Source Code</a>
					</BlogContent>
				</Grid>
			))}
		</Grid>
